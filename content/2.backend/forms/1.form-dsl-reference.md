...
- `maxSize(int)`
- `maxFiles(int)`
- `maxTotalSize(int)` in bytes for all selected files combined
- `storageDisk(string)`
- `storageDirectory(string)`
- `visibility('public'|'private')`

`maxSize` limits each file individually. `maxFiles` limits the number of files when `multiple(true)` is enabled. `accept` accepts extensions (for example `.pdf`) and MIME types (for example `image/*`).

...
- field and rule failures keep the same structure across locales

Rule-specific validation messages are translated server-side, including required checks, length limits, date bounds, boolean checks, and option-based failures. Clients should render the returned message directly instead of trying to recompose it from technical rule names.
